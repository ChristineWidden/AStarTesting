   ...
   private boolean findGoal(Point pos, Point goal, GridValues[][] grid, List<Point> path)
   {
      ...
      //{
         points = strategy.computePath(pos, goalPos,
                              p ->  withinBounds(p, grid) && grid[p.y][p.x] != GridValues.OBSTACLE, //not outside grid, not obstacle
                              (p1, p2) -> neighbors(p1,p2), //are these 2 points neighbors
                              //PathingStrategy.CARDINAL_NEIGHBORS); // a stream of all possible surrounding locations
                              //DIAGONAL_NEIGHBORS);
